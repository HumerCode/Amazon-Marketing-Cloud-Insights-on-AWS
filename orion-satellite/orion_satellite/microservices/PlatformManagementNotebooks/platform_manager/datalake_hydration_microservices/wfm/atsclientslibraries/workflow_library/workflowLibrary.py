...
def set_workflow_record(workflow_details, ATS_TEAM_NAME):
    ...
    wf_library_table = dynamodb_client_wr.Table(f'wfm-{ATS_TEAM_NAME}-dlhs-AMCWorkflowLibrary')
...
def get_workflow_record(ATS_TEAM_NAME):
    ...
    dynamodb_resp_rd = dump_table(table_name=f'wfm-{ATS_TEAM_NAME}-dlhs-AMCWorkflowLibrary', dynamodb_client_rd=dynamodb_client_rd)
...
def delete_workflow_record(workflowId, ATS_TEAM_NAME):
    dynamodb_client_wr= boto3.resource('dynamodb')
