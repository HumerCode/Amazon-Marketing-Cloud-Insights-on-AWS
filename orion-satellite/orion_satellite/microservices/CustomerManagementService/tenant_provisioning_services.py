...
class TenantProvisiongService(Stack):
        ...
        **kwargs: Any,
        ...
        self._artifacts_bucket_key: IKey = Key.from_key_arn(
            ...
            key_arn=get_ssm_value(
                ...
                parameter_name="/Orion/KMS/ArtifactsBucketKeyArn",
            ),
        )
    ...
    def _sync_cfn_template_s3_bucket(self) -> None:
        ...
        self._template_path: str = "orion_satellite/microservices/CustomerManagementService"
    ...
    #    SSM Parameter for sqs sns kms arn
        StringParameter(
            self,
            f"orion-{self._microservice_name}-sqs-sns-ingegration-key",
            parameter_name=f"/Orion/KMS/{self._team}/{self._microservice_name}/SQSSNSKMSKey",
    ...
    def _create_ddb_table(self, ddb_name):
        ...
        StringParameter(
            ...
            parameter_name=f"/Orion/DynamoDB/{self._microservice_name}/{self._team}/CustomerConfigTableArn",
    ...
    def _create_sqs_queue(self,sqs_name):
        ...
        StringParameter(
            self,
            f"orion-{sqs_name}-queue-DLQ.fifo",
            parameter_name=f"/Orion/SQS/{self._microservice_name}/{self._team}/CustomerAMCInstanceOnboardingDLQ",
            string_value=dlq_name,
        )

        self._onboarding_queue = Queue(
            self, 
            id=queue_name, 
        ...
        StringParameter(
            self,
            f'orion-{sqs_name}-queue.fifo',
            parameter_name=f"/Orion/SQS/{self._microservice_name}/{self._team}/CustomerAMCInstanceOnboardingQueue",
    ...
    def _create_sns_topic(self, topic_name):
        ...
        StringParameter(
            self,
            f"orion-{self._microservice_name}-{self._team}-sns-customer-config-topic-ssm",
            parameter_name=f"/Orion/SNS/{self._microservice_name}/{self._team}/CustomerConfigTopicName",
    ...
    def _create_lambdas(self,microservice, team, environment_id):
        ...
        self._lambda_amc_instance_setup.add_to_role_policy(
                PolicyStatement(
                    ...
                    resources=[
                        f"arn:aws:states:{core.Aws.REGION}:{core.Aws.ACCOUNT_ID}:stateMachine:{microservice}-{team}-*"
                    ],
                )
            )
        self._lambda_amc_instance_setup.add_to_role_policy(
                PolicyStatement(
                    ...
                    resources=[
                        f"arn:aws:sqs:{core.Aws.REGION}:{core.Aws.ACCOUNT_ID}:{microservice}-{team}-{environment_id}-*"
                    ],
                )
            )
        self._lambda_amc_instance_setup.add_to_role_policy(
                PolicyStatement(
                    ...
                    resources=[
                        f"arn:aws:ssm:{core.Aws.REGION}:{core.Aws.ACCOUNT_ID}:parameter/Orion/*"
                    ],
                )
            )
    ...
    def _create_amc_onboarding_sm(self, microservice, team, environment_id):
        ...
        add_amc_instance_policy=PolicyDocument(
            statements=[
                ...
                PolicyStatement(
                    ...
                    resources=[
                        f"arn:aws:kms:{core.Aws.REGION}:{core.Aws.ACCOUNT_ID}:key/*",
                        f"arn:aws:kms:{core.Aws.REGION}:{core.Aws.ACCOUNT_ID}:alias/*"
                    ],
                ),
                PolicyStatement(
                    effect=Effect.ALLOW,
                    ...
                    resources=[
                        f"arn:aws:iam::{core.Aws.ACCOUNT_ID}:role/service-role/{self._microservice_name}*"
                    ],
                ),
                PolicyStatement(
                    effect=Effect.ALLOW,
                ...
                PolicyStatement(
                    ...
                    resources=[
                        f"arn:aws:cloudformation:{core.Aws.REGION}:{core.Aws.ACCOUNT_ID}:stack/orion-*",
                        f"arn:aws:cloudformation:{core.Aws.REGION}:{core.Aws.ACCOUNT_ID}:stack/{self._microservice_name}-*",
                        f"arn:aws:cloudformation:{core.Aws.REGION}:aws:transform/*"
                    ],
                ),
                PolicyStatement(
                    effect=Effect.ALLOW,
        ...
        self._amc_instance_post_deploy_metadata.add_layers(orion_layer_version)
        ...
        self._amc_instance_post_deploy_metadata.add_to_role_policy(
                PolicyStatement(
                    ...
                    resources=[
                        ...
                        f"arn:aws:dynamodb:{core.Aws.REGION}:{core.Aws.ACCOUNT_ID}:table/sas-*"
                    ],
                )
            )
        ...
        ManagedPolicy(
            ...
            document=PolicyDocument(
                statements=[
                    ...
                    resources=[
                        f"arn:aws:lambda:{core.Aws.REGION}:{core.Aws.ACCOUNT_ID}:function:orion-tps-*",
                        f"arn:aws:lambda:{core.Aws.REGION}:{core.Aws.ACCOUNT_ID}:function:tps-*"
                    ],
                    ),
                    PolicyStatement(
                    effect=Effect.ALLOW,
                    ...
                    resources=[
                        f"arn:aws:events:{core.Aws.REGION}:{core.Aws.ACCOUNT_ID}:rule/StepFunctionsGetEventsForStepFunctionsExecutionRule",
                    ],
                    )
                ]
            )
        ...
        StringParameter(
            ...
            parameter_name=f"/Orion/SM/{microservice}/{team}/InitializeAMC",
