...
if ! $nflag
then
    STACK_NAME="platform-notebook-manager"
fi
...
aws cloudformation package --profile $PROFILE --template-file $DIRNAME/platform_manager_sagemaker.yaml --s3-bucket $S3_BUCKET --s3-prefix platform_notebook_manager --output-template-file $DIRNAME/output/packaged-template.yaml

unzip GettingStarted_Platform_manager_notebooks.zip
aws s3 sync ./platform_manager/ s3://$S3_BUCKET/platform_notebook_manager_samples/platform_manager/
