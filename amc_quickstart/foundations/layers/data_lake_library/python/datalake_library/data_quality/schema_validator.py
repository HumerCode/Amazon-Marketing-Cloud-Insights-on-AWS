...
import boto3
import awswrangler as wr  # Ensure Lambda has an AWS Wrangler Layer configured1
...
class GlueSchemaValidator(ABC):
    ...
    def __init__(self, boto3_session=None):
        ...
        self.glue_client = boto3_session.client(
            'glue') if boto3_session else boto3.client('glue')
    ...
    def _get_table_parameters(self, database_name, table_name):
        ...
        return self.glue_client.get_table(
            DatabaseName=database_name, Name=table_name)['Table']['Parameters']
    ...
    def _get_table_schema(self, database_name, table_name):
        ...
        """
        return self.glue_client.get_table(
            DatabaseName=database_name, Name=table_name)['Table']['StorageDescriptor']['Columns']
...
class ParquetSchemaValidator(GlueSchemaValidator):
    def validate(self, prefix, keys, database_name, table_name):
        ...
        table_parameters = self._get_table_parameters(
            database_name, table_name)
        logger.info(f"Table parameters: {table_parameters}")

        # Parse table parameters
        validate_schema, validate_latest = self._parse_table_parameters(
            table_parameters)
        ...
        if validate_schema:
            ...
            table_schema = sorted(
                self._get_table_schema(database_name, table_name), key=lambda x: x['Name'])
            logger.info(f"Table schema: {table_schema}")

            # Retrieve object schema
            object_schema = self._get_object_schema(
                prefix, keys, validate_latest)
            logger.info(
                f"Object prefix: {prefix}, keys: {keys}, schema: {object_schema}")
    ...
    def _get_object_schema(self, prefix, keys, get_latest):
        ...
        s3_objects = wr.s3.describe_objects(
            path=keys if keys and len(keys) > 0 else prefix,
            boto3_session=self.boto3_session
        )

        # Sort by last modified date and filter out empty objects
        object_keys = [object_key for object_key, object_metadata in sorted(
            s3_objects.items(),
            key=lambda k_v: k_v[1]['LastModified']
        ) if object_metadata['ContentLength'] > 0]

        # Retrieve Parquet metadata
        column_types, _ = wr.s3.read_parquet_metadata(
            path=object_keys[0] if get_latest else object_keys,
            boto3_session=self.boto3_session
        )
        ...
        # does not respect the order
        return sorted(
            list({'Name': name.lower(), 'Type': type}
    ...
    def _parse_table_parameters(table_parameters):
        ...
        try:
            validate_schema = True \
                if table_parameters and table_parameters['validate_schema'] == 'true' else False
        except KeyError:
            validate_schema = False
        try:
            validate_latest = True \
                if table_parameters and table_parameters['validate_latest'] == 'true' else False
        except KeyError:
