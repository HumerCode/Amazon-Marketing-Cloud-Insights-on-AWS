...
function with_backoff {
  ...
  if [[ $exitCode != 0 ]]
  then
    echo "Command ($@) failed all attempts\n Please check that IAM role sdlf-cicd-team-codecommit-${ENV}-${TEAM_NAME}-${AWS_REGION} exists in the DevOps account" 1>&2
    exit 1
  fi
...
with_backoff aws iam get-role --role-name sdlf-cicd-team-codecommit-${ENV}-${TEAM_NAME}-${AWS_REGION} --profile crossaccount