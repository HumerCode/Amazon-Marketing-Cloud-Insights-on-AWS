...
## Adding glue jobrole as data location permission
stg_bucket=$(sed -e 's/^"//' -e 's/"$//' <<<"$(aws ssm get-parameter --name /SDLF/S3/StageBucket --profile $PROFILE --query "Parameter.Value")")
gluejobrole=$(sed -e 's/^"//' -e 's/"$//' <<<"$(aws ssm get-parameter --name /SDLF/IAM/$TEAM_NAME/GlueJobRoleArn --profile $PROFILE --query "Parameter.Value")")
s3_loc=$(sed -e 's/^"//' -e 's/"$//' <<<"{ \"DataLocation\": {\"ResourceArn\":\"arn:aws:s3:::$stg_bucket/post-stage/$TEAM_NAME/\"}}")
aws lakeformation grant-permissions --principal DataLakePrincipalIdentifier=$gluejobrole --permissions "DATA_LOCATION_ACCESS" --resource "$s3_loc" --profile $PROFILE
datalakeuserrole=$(sed -e 's/^"//' -e 's/"$//' <<<"$(aws ssm get-parameter --name /SDLF/IAM/$TEAM_NAME/DataLakeUsersArn --profile $PROFILE --query "Parameter.Value")")
db_stg_name="$TEAM_NAME"_"$DATASET_NAME"_"$ENV"_stage
echo $TEAM_NAME
echo $DATASET_NAME
echo $ENV
echo $db_stg_name
db_stage_name=$(sed -e 's/^"//' -e 's/"$//' <<<"{ \"Table\": {\"DatabaseName\":\"$db_stg_name\",\"TableWildcard\": {}}}")
echo $db_stage_name
aws lakeformation grant-permissions --principal DataLakePrincipalIdentifier=$datalakeuserrole --permissions "ALL" --resource "$db_stage_name" --profile $PROFILE