...
. Sign in to your AWS account, and choose from the following options to launch the AWS CloudFormation template. For help with choosing an option, see link:#_deployment_options[Deployment options], earlier in this guide.
+
[cols="3,1"]
|===
^|https://fwd.aws/XYDGJ?[Deploy {partner-product-short-name} into a new or an existing AWS account^]
^|https://fwd.aws/4JePw?[View template^]

//^|https://fwd.aws/9GwNE?[Deploy {partner-product-short-name} into an existing VPC on AWS^]
//^|https://fwd.aws/VYJkV?[View template^]
|===
+
