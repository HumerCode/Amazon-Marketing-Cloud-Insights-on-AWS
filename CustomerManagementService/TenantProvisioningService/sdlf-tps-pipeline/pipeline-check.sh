...
while true;
do
  if ! aws codepipeline get-pipeline-state --name sdlf-${TEAM}-cicd-tps-pipeline; then
    sleep 5
...
<<<"$(aws codepipeline get-pipeline-state --name sdlf-${TEAM}-cicd-tps-pipeline)")" = InProgress ]
do
sleep 60
COUNT=$((COUNT+1))
