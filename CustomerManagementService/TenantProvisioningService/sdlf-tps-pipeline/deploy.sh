...
MICROSERVICE_NAME=$(sed -e 's/^"//' -e 's/"$//' <<<"$(jq '.[] | select(.ParameterKey=="pMicroserviceName") | .ParameterValue' $DIRNAME/parameters-$ENV.json)")
...
if ! $nflag
then
    STACK_NAME="sdlf-$MICROSERVICE_NAME-$PIPELINE_NAME"
fi
...
aws cloudformation package --profile $PROFILE --template-file $DIRNAME/template.yaml --s3-bucket $S3_BUCKET --s3-prefix $MICROSERVICE_NAME/pipeline/$PIPELINE_NAME --output-template-file $DIRNAME/output/packaged-template.yaml

if [ "$(ls -A $DIRNAME/scripts/*.yaml)" ]; then
    echo "Uploading files from $DIRNAME/scripts/ to artifactory"
    for file in $DIRNAME/scripts/*.yaml
    do
        echo "Uploading $file to $S3_BUCKET/$MICROSERVICE_NAME/scripts/$PIPELINE_NAME/"
        aws s3 cp $file s3://$S3_BUCKET/$MICROSERVICE_NAME/scripts/$PIPELINE_NAME/ --profile $PROFILE
    done
fi
